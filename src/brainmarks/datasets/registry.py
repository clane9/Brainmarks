...
def import_dataset_plugins():
    ...
    plugins = {}
    for finder, name, ispkg in pkgutil.iter_modules(brainmarks.datasets.__path__):
        if not (name in {"base", "registry", "template"} or name.startswith("test_")):
            try:
                plugins[name] = importlib.import_module(f"brainmarks.datasets.{name}")
