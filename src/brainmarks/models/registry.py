...
import brainmarks.models
from brainmarks.models.base import ModelTransformPair, ModelFn
...
def import_model_plugins():
    ...
    plugins = {}
    for finder, name, ispkg in pkgutil.iter_modules(brainmarks.models.__path__):
        if not (name in {"base", "registry", "template"} or name.startswith("test_")):
            try:
                plugins[name] = importlib.import_module(f"brainmarks.models.{name}")
